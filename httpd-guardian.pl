...
#         1) Modify: my $PROTECT_EXEC = "/bin/echo %s"; 
#         2) Then run from CLI: cat /path-to-modsecurity2-log-dir/modsec_guardian.log | ./httpd-guardian.pl
#        
#     To configure ModSecurity to work with this script, without using 'www-security-assistant.bash' (Iptables chain GUARDIAN is required):
#         1) Modify: my $PROTECT_EXEC = "/sbin/iptables -A GUARDIAN -s %s -j DROP && /bin/echo /sbin/iptables -D GUARDIAN -s %s -j DROP | at now + 5 minutes";
#         2) Edit '/etc/modsecurity/modsecurity.conf' and pipe the Guardian log to the script: SecGuardianLog "|/path-to/httpd-guardian.pl"
#
...
#
# --- End of the Redistribution Notes --- Currently there is no other modifications ----
